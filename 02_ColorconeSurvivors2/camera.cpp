...
#define CAMERA_HEIGHT	(2500.0f)	//高さ
...
CCamera::CCamera()
{
	...
	m_posV = D3DXVECTOR3(0.0f, CAMERA_HEIGHT, 0.0f);
...
void CCamera::Update(void)
{
	...
	//慣性
	m_posVDest.x = PlayerPos.x + sinf(m_rot.y) * -DIS;		//目的の視点にモデルの位置を代入(相対座標)
	m_posV.y = DIS;
	m_posVDest.z = PlayerPos.z + cosf(m_rot.y) * -DIS;		//目的の視点にモデルの位置を代入(相対座標)
	(m_posVDest - m_posV) * 0.1f;

	//カメラ追従処理
	m_posR = PlayerPos;		//注視点にモデルの位置を代
